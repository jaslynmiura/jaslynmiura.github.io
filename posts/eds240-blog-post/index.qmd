...
**Sub-questions**: Which countries experience the most natural disasters per year around the world? What type natural disasters have occurred the most in different countries? What are the affects of natural disasters on the populations of different countries?
...
My first plot depicts the trend of annual natural disasters. This is a simple plot that coneys the impactful messages that natural disasters have dramatically increased since 1980.

![](images/line_trend.png)

## Total Natural Disasters by Country

Next, I wanted to determine which countries around the world have the highest total natural disasters. To help the audience identify key points, I also plotted the totals next to the bar charts. This makes it easier for the auidence to undestand the differences between the countries. By adding the flags to the bar chart, I think this helps the audience identify countries and adds some color to the chart.

![](images/top10.png)

## Disaster Events by Population

I then wanted to understand how the different population sizes of each country are affected by natural disasters. Here, I wanted to highlight the fact that smaller countries such as the Philippines experience much more disasters relative to population size. Since the Philippines experiences drastically more disasters relative to population size but is ranked second in the amount of natural disasters they experience, I wanted to highlight this difference. Because, we're again looking at the top ten countries, I wanted to keep the theme of the bar plots similar.

![](images/pop_bar.png)

## Population Displaced from Disasters (2008-2018)

I created this waffle chart to understand additional human impacts of natural disasters, specifically the amount of the population internally displaces due to natural disasters. This required taking the average yearly population size and average yearly displacement size. I also did some calculations so that a square would reflect "1,000 displaced people per million', which is a much more understandable scale for the audience to comprehend. To elevate these plots, I used affinity to shape the waffle grid as each country's border.
...
**Typography:** I chose to use Noto Serif Myanmar for the titles and headers within my infographic. I used Red Hat Mono for all other text within my infographic, such as axis labels, supporting text, etc. I decided to use these two fonts because they have an academic feel.
...
pop_bar <- ggplot(disaster_percap, aes(x = dis_per_million, y = country)) +
  ...
  # Add the total value to the end of the bar.
  geom_text(aes(label = scales::comma(dis_per_million)),
            hjust = -0.55,
            color = "black",
            size = rel(1),
            family = "red") +
  # Add flag images to associated country.
  geom_image(aes(image = flag), 
             x = max(top10_percap$dis_per_million) * 0.035,
             size = 0.05,
             by = "width") +
  # Bars are colored by true/false statements with designated colors.
  scale_fill_manual(values = c("TRUE" = "#000080", "FALSE" = "#B8B8B8"),
                    guide = "none")  +
  # Expand the x-axis for for flag and total value placement.
  scale_x_continuous(labels = scales::label_comma(),
...
stacked_bar <- ggplot(data = type_country, aes(x = country, y = disastertype_count, fill = disastertype)) +
...
Internal Displacement Monitoring Centre (IDMC). Internally displaced persons, new displacement associated with disasters (number of cases) [WB_WDI]. World Bank Group: World Development Indicators. [https://data.worldbank.org/indicator/VC.IDP.NWDS](https://data.worldbank.org/indicator/VC.IDP.NWDS) Date Accessed: 2026-02-25

Rosvold, E., & Buhaug, H. (2021). Geocoded Disasters (GDIS) Dataset (Version 1.00) [Data set]. Palisades, NY: NASA Socioeconomic Data and Applications Center (SEDAC). [https://doi.org/10.7927/ZZ3B-8Y61](https://doi.org/10.7927/ZZ3B-8Y61) Date Accessed: 2026-02-19

World Population Prospects, United Nations (UN), et. al. Population, total [WB_WDI]. World Bank Group: World Development Indicators. [https://data.worldbank.org/indicator/SP.POP.TOTL](https://data.worldbank.org/indicator/SP.POP.TOTL) Date Accessed: 2026-02-25
